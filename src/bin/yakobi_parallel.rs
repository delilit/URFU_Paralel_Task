...
let mut counts = 0;
    for _step in 0..steps{
        counts+=1;
        vec_copy.par_iter_mut().enumerate().for_each(|(i, row)| {
            if i == 0 || i == n-1{
                return;
            }
            for j in 1..n-1{
                let x = i as f32/u;
                let y = j as f32/u;
                row[j] = 0.25*(vec[i-1][j] + vec[i+1][j] + vec[i][j-1] + vec[i][j+1] - h*h*f(x,y));
            }
        });
        std::mem::swap(&mut vec, &mut vec_copy);
        if counts % 2000 == 0{
            let difference = compare_offense(&p_vec, &vec_copy);
            println!("{} iteration, max difference between real and close result is: {}", counts, difference);
            if difference.abs() <= 0.001 {break;}
        }
    }
